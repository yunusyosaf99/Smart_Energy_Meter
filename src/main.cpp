...
#include <FirebaseESP32.h>          // library to communicate with Firebase
...
#define URL_fw_Version "https://raw.githubusercontent.com/Greenwend-Energy/firmware_version_update/main/EM3P_1V_60A.txt"
#define URL_fw_Bin "https://raw.githubusercontent.com/Greenwend-Energy/firmware_version_update/main/EM3P_1V_60A_fw.bin"
...
const int restartInterval =  6*60*60 * 1000;  // 12 hours in milliseconds
unsigned long lastRestartTime = 0;


#define wifiled 25
#define site_relay 14
// the chip select pins for three ATM90E32 IC
const int CS_pin1 = 5;
const int CS_pin2 = 4;

//the gain values for the voltage and current channels
unsigned short VoltageGain1 = 57343;     
unsigned short VoltageGain2 = 57343;       

unsigned short CurrentGainCT1 = 8817;  
unsigned short CurrentGainCT2 = 8817; 
unsigned short CurrentGainCT3 = 8817; 
unsigned short CurrentGainCT4 = 8817;
unsigned short CurrentGainCT5 = 8817; 
unsigned short CurrentGainCT6 = 8817; 
/*
...
String Timest  = "/ Time_stamp";
String GVAPath = "/Grid_voltage_A";
String Firmvere = "/Active_Firmware_version";
...
String connPath = "/Connection_Type";
...
void firmwareUpdate(void) {
  ...
  case HTTP_UPDATE_FAILED:
    //f("HTTP_UPDATE_FAILD Error (%d): %s\n", httpUpdate.getLastError(), httpUpdate.getLastErrorString().c_str());
    break;

  case HTTP_UPDATE_NO_UPDATES:
    //ln("HTTP_UPDATE_NO_UPDATES");
    break;
...
{
  {
    ...
    server.on("/setting", []() {
      ...
      if ((qsid.length() > 0) && (qpass.length() > 0) && (qemail.length() > 0) && (qupass.length() > 0)) {
        ...
        //ln("writing eeprom user password:");
...
{  // This code is the setup() function in esp32 programming.
  ...
  //ln(FirmwareVer);
  //ln("Start ATM90E32"); // prints "Start ATM90E32" to the serial output
  ...
  //ln("Disconnecting previously connected WiFi");
 ...
 int connectionAttempts = 0;
  while ((WiFi.status() != WL_CONNECTED)) // while loop will repeatedly check the status of the Wi-Fi connection
  {
    ...
    //("."); //  a dot is printed to the serial monitor to indicate that the code is still attempting to connect to Wi-Fi.
  ...
  */
  ...
  Home_importunits = values[3];
   ...
   //ln("Firmware update Starting..");

   pinMode(site_relay,OUTPUT);
...
void loop() {
  ...
  // Check if it's time to call the repeatedCall() function
  if (currentMillis - previousMillis_2 >= 24*6*600000) {
    previousMillis_2 = currentMillis;
    repeatedCall();
  }
  ...
  if (buttonPressed) { //  If the button is pressed, the code will execute the following block of code.
    ...
    if (pressDuration > LONG_PRESS_TIME) {
      ...
      //ln("A long press is detected");
      //ln("Turning the HotSpot On");
  ...
  if (currentMillis - lastConnectionTime >= connectionInterval) {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      ...
      esprestartflag++;
      //ln("WiFi disconnected. Reconnecting...");
      WiFi.disconnect();
      WiFi.begin(esid.c_str(), epass.c_str());
      int count = 0;
      while (WiFi.status() != WL_CONNECTED) {
  ...
  // Update database path
  databasePath = "/UsersData/" + uid + "/readings";
  databasePath1 = "/Firmware_Version/"+ uid   ;
  // Initialize a NTPClient to get time
  //ln(databasePath);
  timeClient.begin();
  // Set offset time in seconds to adjust for your timezone, for example:
...
//added by azman
     ...
     json1.set(Firmvere.c_str(), FirmwareVer);
      //ln("okokokoko");
   

      if (Firebase.RTDB.setJSON(&fbdo1, parentPath1.c_str(), &json1)) {
        //ln("Firmware data sent to parentPath1");
        counter = 0;
      } else {
        //ln("Failed to send firmware data to parentPath1");
        //ln("REASON: " + fbdo1.errorReason());
      }
      parentPath= databasePath + "/" + String(dayStamp)+ "/" + String(timeStamp);
     
      
      json.set(Timest.c_str(), timeStamp);
      ...
      //ln("Voltage: "+String(Acin_voltageA));
      ...
      //ln("Current: "+ String(Acout_currentA));
      ...
      json.set(connPath.c_str(), String("OnGrid"));
      ...
      Acin_exportunits = 0;
      Home_importunits = 0;
        //ln("PATH: " + fbdo.dataPath());
        //ln("TYPE: " + fbdo.dataType());
      }
      else {
        //ln("FAILED");
        //ln("REASON: " + fbdo.errorReason());
      }
    }
    ...
    delay(2000);
    restart();
    if(Control_site() == false)
    {
      digitalWrite(site_relay,1);
    }else
    {
      digitalWrite(site_relay,0);
    }
...
void restart()
{
  ...
  if (millis() - lastRestartTime >= restartInterval) {
    //ln("Restarting ESP32...");
    delay(1000);  // Give some time for the serial monitor to print
...
bool Control_site()
{
  ...
  // Fetch boolean data from Firebase
  String path = "/Appuser/" + uid + "/active";

  if (Firebase.RTDB.getString(&fbdo, path.c_str())) {
    String activeDataString = fbdo.stringData();
    bool activeData = activeDataString.equals("true");
